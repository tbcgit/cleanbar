...
//----------------------------------------------------------------
//  Programmed by: Vicente Arnau.  19 - VI - 2025        
//----------------------------------------------------------------
...
// CB   -l 55  -s  99 -nb 20 barcodes_20.txt  Atrandi_1k.fq
// CB   -l 70  -s 200 -sb 10 barcodes.txt  Atrandi_4k.fastq
...
#define SS          32  //--> maximum label size of BARCODE
...
int main( int argc, char *argv[])
{
 ...
 //--------------------------------->> default values:
 int  HEAD    = 88;    //-->> number of nucleotides analyzed at the start of the read
 int  LLL_VEO = 2000;  //-------> number of predefined reads with screen output
 int  MAXBAR  = 24;   //--> Number of BARCODES per position
 int  BARSIZE =  8;   //--> Number of nt of BARCODE
 int  LINKSIZE=  4;   //--> Number of nt of LINK 
 int  CAB     = 80;    //---> (HEAD-BARSIZE)
  
 float  faux, faux1, faux2;

 char  *ss_com;
 char  **etiAA, **etiBB, **etiCC, **etiDD;  
 char  **SSA, **SSB, **SSC, **SSD; 
 ...
 if (argc == 1) {
   ...
   printf("\n\nYou can use this for mor information: \n./CB  --help \n");
...
//------------->> NO OPTIONS:  only read de files.	 
 if (argc == 3){
	...
	printf ("\n==================================================");
	 ...
	 for (ii=0; ii<options; ii++){
		...
		aux=1;
		aux=  strcmp ( argv[jj], "-s");
		if (aux==0){ //---> we modify the lines per screen that we display
		  	LLL_VEO= atoi(argv[jj+1]);
			printf ("\nLines per screen = %d\n", LLL_VEO);
			}

		aux=1;
		aux=  strcmp ( argv[jj], "-l");
		...
		aux=1;
		aux=  strcmp ( argv[jj], "-bn");
		if (aux==0){ //---> we modify MAXBAR)
			MAXBAR= atoi(argv[jj+1]);
			printf ("\nNumber of BARCODES for group = %d\n", MAXBAR);
			}
		
		//--> ya nt por BARCODE
		aux=1; 
		aux=  strcmp ( argv[jj], "-bs");
		if (aux==0){ //---> we modify MAXBAR)
			BARSIZE= atoi(argv[jj+1]);
			printf ("\nNumber of nt per BARCODES = %d\n", BARSIZE);
			}
		
		CAB = HEAD - BARSIZE;
		
		 //--> falta nt por link: LINKSIZE
		aux=1; 		aux=  strcmp ( argv[jj], "-bs");
		if (aux==0){ //---> we modify MAXBAR)
			LINKSIZE= atoi(argv[jj+1]);
			printf ("\nNumber of nt per LINK = %d\n", LINKSIZE);
			}
 ...
 //----------------->> reserved memory for the barcode labels:

 etiAA = (char **) malloc(MAXBAR * sizeof(char *));
  if (etiAA == NULL) {
    printf("Error allocating memory\n"); return 1;
  }
 for (i=0; i<MAXBAR; i++)
	 etiAA[i]= (char *) malloc(SS * sizeof(char));

 etiBB = (char **) malloc(MAXBAR * sizeof(char *));
  if (etiBB == NULL) {
    printf("Error allocating memory\n"); return 1;
  }
 for (i=0; i<MAXBAR; i++)
	 etiBB[i]= (char *) malloc(SS * sizeof(char));

 etiCC = (char **) malloc(MAXBAR * sizeof(char *));
  if (etiCC == NULL) {
    printf("Error allocating memory\n"); return 1;
  }
 for (i=0; i<MAXBAR; i++)
	 etiCC[i]= (char *) malloc(SS * sizeof(char));

 etiDD = (char **) malloc(MAXBAR * sizeof(char *));
  if (etiDD == NULL) {
    printf("Error allocating memory\n"); return 1;
  }
 for (i=0; i<MAXBAR; i++)
	 etiDD[i]= (char *) malloc(SS * sizeof(char));
 
 
//------->> reserved memory for the BARCODES:
 
 SSA = (char **) malloc(MAXBAR * sizeof(char *));
 if (SSA == NULL) {
    printf("Error allocating memory\n"); return 1;
  }
 for (i=0; i<MAXBAR; i++)
	 SSA[i]= (char *) malloc((BARSIZE+1) * sizeof(char));
 
 SSB = (char **) malloc(MAXBAR * sizeof(char *));
 if (SSB == NULL) {
    printf("Error allocating memory\n"); return 1;
  }
 for (i=0; i<MAXBAR; i++)
	 SSB[i]= (char *) malloc((BARSIZE+1) * sizeof(char));
 
 SSC = (char **) malloc(MAXBAR * sizeof(char *));
 if (SSC == NULL) {
    printf("Error allocating memory\n"); return 1;
  }
 for (i=0; i<MAXBAR; i++)
	 SSC[i]= (char *) malloc((BARSIZE+1) * sizeof(char));
 
 SSD = (char **) malloc(MAXBAR * sizeof(char *));
 if (SSD == NULL) {
    printf("Error allocating memory\n"); return 1;
  }
 for (i=0; i<MAXBAR; i++)
	 SSD[i]= (char *) malloc((BARSIZE+1) * sizeof(char));
 
//--------------------------------------------------------

 ss_com= (char *) malloc((BARSIZE+1) * sizeof(char));
...
//---------------------------
 for (i=0; i<MAXBAR; i++){
	aux= fscanf(fbar,"%s %s", etiAA[i], SSA[i]);
...
//---------------------------
 for (i=0; i<MAXBAR; i++){
	aux= fscanf(fbar,"%s %s", etiBB[i], SSB[i]);
...
//---------------------------
 for (i=0; i<MAXBAR; i++){
	aux= fscanf(fbar,"%s %s", etiCC[i], SSC[i]);
...
//---------------------------
 for (i=0; i<MAXBAR; i++){
	aux= fscanf(fbar,"%s %s", etiDD[i], SSD[i]);
 ...
 while (!feof(ffastq)) {
	...
	if (len2>CAB){//====================>>  TENGO LINEA2  <<===========
	    ...
	    for (i=0; i<MAXBAR; i++){
	       ...
	       dd= busca(SSD[i], linea2, 0, CAB, BARSIZE);  
  ...
  if (lim <= LLL_VEO) //-->> por pantalla las primeras LLL_VEO reads
	...
	i_barD=0; dd= 0;  min_dd= CAB_4; //--> UN NUMERO GRANDE.
	    ...
	    for (i=0; i<MAXBAR; i++){
		   ...
		   //--> Funcion de busqueda de BARCODES:
	       dd = busca(SSD[i], linea2_inv, 0, CAB, BARSIZE);  
...
// BARSIZE is the size of the BARCODE searched
